    ...
    <>
      ...
      {team.category !== "Prima Squadra" && (
        <Link
          ...
          className="group bg-brand-red hover:bg-brand-red-dark focus-visible:outline-brand-gold block transition-colors focus-visible:outline-2 focus-visible:-outline-offset-2"
        >
          <Container
            size="wide"
            className="flex min-h-16 items-center justify-between gap-4 lg:min-h-20"
          >
            {/* Su fondo rosso l'oro scende a 2.3:1: icona, testo e freccia
                vanno tutti in brand-white (4.5:1 a questa dimensione). */}
            <span className="font-display text-brand-white flex items-center gap-3 text-xl font-extrabold tracking-[0.12em] uppercase md:text-2xl">
              <CalendarDays
                size={24}
                className="shrink-0"
                aria-hidden
            ...
            <ArrowRight
              size={22}
              className="text-brand-white shrink-0 transition-transform group-hover:translate-x-1"
              aria-hidden
            />
          </Container>
        </Link>
